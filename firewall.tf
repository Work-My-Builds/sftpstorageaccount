...
resource "azurerm_firewall" "firewall" {
  ...
  depends_on = [
    azurerm_firewall_policy_rule_collection_group.rule_collection_group
  ]
}
...
resource "azurerm_firewall_policy_rule_collection_group" "rule_collection_group" {
  ...
  nat_rule_collection {
    ...
    rule {
      ...
      translated_port     = 22
      translated_address  = azurerm_private_endpoint.blob_private_endpoint.private_service_connection[0].private_ip_address
